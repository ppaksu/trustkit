// 게이트키퍼 SDK 테스트. 명세 docs/DESIGN.md 3.1절, 4.5절.
...
import { Gatekeeper, evaluate, policyHash, type Policy, type TxRequest } from "../sdk/gatekeeper.ts";
...
test("정책 — 통과 요청은 아무 기록도 남기지 않는다", async () => {
  const g = new Gatekeeper({ account: gk, domain: D, policy: POLICY, now: () => NOW });
...
test("정책 — 먼저 일치하는 규칙이 이긴다", () => {
  ...
  assert.equal(d.allow === false && d.rule_id, "DENYLIST_SANCTIONED");
  assert.equal(d.allow === false && d.severity, "block");
});
...
test("정책 — 세 규칙이 각각 다른 심각도를 낸다", () => {
  ...
  assert.equal(a.allow === false && a.severity, "block");
  assert.equal(b.allow === false && b.severity, "hold");
  assert.equal(c.allow === false && c.severity, "review");
});

test("정책 — 규칙 순서를 바꾸면 policy_hash 가 달라진다", () => {
  const swapped: Policy = { ...POLICY, denylist: [...POLICY.denylist].reverse(), version: 2 };
...
test("영수증 — 거절이면 서명된 영수증이 나온다", async () => {
  const g = new Gatekeeper({ account: gk, domain: D, policy: POLICY, now: () => NOW });
  const r = await g.handle(req({ target: SANCTIONED }));
...
test("영수증 — 판정 사유가 커밋되고 선택적으로 공개된다", async () => {
  const g = new Gatekeeper({ account: gk, domain: D, policy: POLICY, now: () => NOW });
  const receipt = (await g.handle(req({ target: SANCTIONED }))).receipt!;
...
test("영수증 — calldata 해시가 원문 calldata 에서 계산된다", async () => {
  ...
  const receipt = (await g.handle(req({ target: SANCTIONED, calldata }))).receipt!;
...
test("책임 — 명세 4.5절 표를 그대로 재현한다", async () => {
  const g = new Gatekeeper({ account: gk, domain: D, policy: POLICY, now: () => NOW });
  const receipt = (await g.handle(req({ target: SANCTIONED }))).receipt!;

  assert.equal(assignFault(null, false, NOW), "증명 불가");
  assert.equal(assignFault(receipt, false, NOW), "게이트키퍼가 제출 안 함");
